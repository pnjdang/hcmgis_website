...
//        ]);
    ...
    public function actionView(){
        ...
        $allPosts = GisPosts::find()->where(['<>', 'id', $model->id])->andWhere(['post_type' => 'post'])->andWhere(['ping_status' => 1])->all();
        shuffle($allPosts); // Shuffle the array of GisPosts objects
        $dalienquan = array_slice($allPosts, 0, 5); // Pick the first 5 items
        if($model == null){
            return $this->render('notfound');
        }
        return $this->render('view',[
            'model' => $model,
            'dalienquan' => $dalienquan
        ]);
    }
}
